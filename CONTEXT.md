...
Confers **authority**: states what its subject may *do*, as itself, within a named `scope` governed by the issuer, as an explicit list of `actions` (empty confers nothing; no action implies another). Stands outside the three informative categories. Top-level `id` is REQUIRED. May be **attenuated** by its holder without the governing party: a child VAC's `issuer` MUST be its parent's `credentialSubject.id`, and it may only narrow actions, scope and validity. The holder presents the whole chain (verifiers MUST NOT dereference `parent`); depth is capped at 8. Authority is not membership: proving both in zero knowledge requires a shared-subject proof. An agent equipped to act as itself gets an attenuated VAC; an agent that is to act as its principal gets a VDC.
