...
Methods that publish a verifiable, append-only history of DID document versions — such as [did:webvh](https://identity.foundation/didwebvh/), whose log entries commit to successor keys and may be countersigned by *DID log witnesses* — satisfy these properties. A DID log witness countersigns versions of a DID document and is a distinct role from the witness of a [[ref: VWC]], which attests to an edge; the two share only a name, and this subsection uses the qualified term wherever the DID log sense is meant. Methods that resolve to a document with no verifiable history, such as `did:web`, satisfy the last property only; a verifier can establish the current key but not the key that was authoritative at issuance, and there is no recovery path on compromise.
