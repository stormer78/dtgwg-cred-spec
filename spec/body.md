...
A witnessed exchange of a complete [[ref: DTG edge]] is bidirectional: two edge credentials, one in each direction, are formed in a single witnessing event — two VRCs for a peer-to-peer edge, or the two VMCs of a membership edge. For such exchanges the witness SHOULD issue one VWC per direction. In each VWC, `credentialSubject.id` MUST be the DID of the issuer of the edge credential that the VWC attests (the credential referenced by `digest`, when present), so that the two VWCs of an exchange are unambiguously bound to their respective directions.
...
- `credentialSubject` (object, REQUIRED):
  ...
  - `digest` (string, OPTIONAL): A cryptographic hash of the witnessed edge credential. The hash MUST be computed as the SHA-256 hash of the credential's JSON representation canonicalized with the JSON Canonicalization Scheme ([JCS, RFC 8785](https://datatracker.ietf.org/doc/html/rfc8785)), and MUST be encoded as the string `sha256:` followed by the lowercase hexadecimal digest.
...
4. **Digest integrity.** When a VWC includes a `digest` of the witnessed edge credential, or a member-issued VMC includes a `digest` of the community-issued VMC it acknowledges, verifiers relying on the credential should recompute the SHA-256 hash over the JCS (RFC 8785) canonical form of the referenced credential and confirm the match; a mismatch invalidates the attestation or the acknowledgement.
