...
2. The verifier can establish that the credential's issuer holds a [[ref: VMC]] issued by a [[ref: VTC]] in the anchor set that verifier accepts.
...
Condition 2 MAY be satisfied by either of two routes, which are equivalent for the purposes of this section:

- **By disclosure**, where the credential carries [[ref: M-DIDs]] whose VMCs the verifier resolves and checks against its anchor set.
...
9. **Effective disclosure of an edge.** A choice of identifier constrains only the disclosure of the party who makes it. Each half of an edge is issued by its own party, under an identifier that party chose, and published wherever that party publishes it; neither party controls, and neither can necessarily discover, what the other does with its own half. An edge's effective disclosure is therefore the wider of its two halves, not the narrower. A [[ref: R-DID]] that is correctly unique to one counterparty is still correlated to a named party if that counterparty publishes the opposing half under an [[ref: M-DID]] into a graph others can read — a different failure from the reuse addressed above, and one the party who followed this specification's recommendation cannot prevent. Implementations should compute the privacy of an edge over every place either half is published, not from the half they issued, and should not represent a pairwise half as making the edge pairwise. The halves of a single edge may legitimately differ in this respect, both across communities whose disclosure policies differ and within one community passing through the migration recommended in item 1. The same reasoning applies to any joint presentation: what a set of credentials discloses together may exceed what any of them discloses alone.
